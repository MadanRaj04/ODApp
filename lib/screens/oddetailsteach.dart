...
class oddetailsteach extends StatefulWidget {
  final String tid;
  ...
  @override
  _oddetailsteachState createState() => _oddetailsteachState();
}

class _oddetailsteachState extends State<oddetailsteach> {
  ...
  bool isLoaded = false;
  late String chkId; // Declare chkId as a global variable
  ...
  Future<void> _fetchData() async {
    ...
    chkId = chk1.id; // Assign the value of chk1.id to chkId
          ...
          if (status.isNotEmpty) ...{
            ...
            Row(
              ...
              children: [
                ElevatedButton(
                  onPressed: () {
                    _updateStatus(index, kval, "Accepted");
                    Navigator.of(context).pop();
                ...
                ElevatedButton(
                  onPressed: () {
                    _updateStatus(index, kval, "Rejected");
                    Navigator.of(context).pop();
  ...
  void _updateStatus(int index, String kval, String newStatus) async {
    // Update status in Firestore
    await FirebaseFirestore.instance.collection('userData').doc(chkId).update({
      '$kval.Status': newStatus,
    });
    // Update local items list to reflect the change
    setState(() {
      items[index][kval]["Status"] = newStatus;
    });
